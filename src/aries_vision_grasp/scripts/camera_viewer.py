...
class CameraViewerNode(Node):
    ...
    def display_callback(self):
        ...
        if self.latest_depth is not None:
            # Handle depth visualization with better range handling
            depth_valid = self.latest_depth.copy()
            
            # Replace invalid values with max valid depth
            valid_mask = ~np.isnan(depth_valid) & ~np.isinf(depth_valid) & (depth_valid > 0)
            if np.any(valid_mask):
                max_valid = np.max(depth_valid[valid_mask])
                depth_valid[~valid_mask] = max_valid
                
                # Normalize with clipping for better contrast
                depth_clipped = np.clip(depth_valid, 0, 10.0)  # Clip to 10m max
                ...
                # Show depth range
                min_depth = np.min(depth_valid[valid_mask])
                max_depth = np.max(depth_valid[valid_mask])
                cv2.putText(
                    depth_colored,
