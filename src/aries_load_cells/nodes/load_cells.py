...
from std_msgs.msg import Bool, Float32, Float64, Int32, Int32MultiArray, String, UInt8
...
# Below this many counts per kilogram, a scale is reported as implausible
# rather than accepted quietly. An HX711 at gain 128 on a cell this size lands
# in the hundreds of thousands, so this is three orders of magnitude of slack --
# it is a "that cannot be right" guard, not a tolerance.
PLAUSIBLE_MIN_SCALE = 1000.0
...
class Cell:
    ...
    def __init__(self, name, scale, offset, invert, filter_samples,
        ...
        self.offset = offset
        # The lid's weight, in RAW COUNTS, measured as the difference from
        # `offset`. Zero until somebody tares with the lid on, so a container
        # weighed without one is unaffected. Counts rather than kilograms so
        # that changing `scale` does not silently invalidate it.
        #
        # NOT PERSISTED. There is no lid_offset in load_cells.yaml, on purpose:
        # it is a property of the current run, not of the hardware, and a lid
        # tare that survived a restart would keep subtracting a lid nobody had
        # put back on.
        self.lid_offset = 0.0
    ...
    def weight(self):
        ...
        if raw is None:
            return float("nan")
        return self.sign * (raw - self.offset - self.lid_offset) / self.scale

    def latest_counts(self):
        """The filtered raw count, or None when there is nothing to report."""
        return self.filtered_raw()

    def tare(self):
        """Zero the cell at whatever is on it now. Returns the new offset.

        Clears any lid tare. A lid weight is a difference from the OLD zero and
        means nothing against a new one, so keeping it here would silently
        subtract a stale constant from every reading afterwards.
        """
        raw = self.filtered_raw()
        if raw is None:
            return None
...
class LoadCells(Node):
    def __init__(self):
        ...
        for name in self.cell_names:
            ...
            elif abs(scale) < PLAUSIBLE_MIN_SCALE:
                # NOT A PLACEHOLDER, BUT NOT PLAUSIBLE EITHER. The three values
                # in load_cells.yaml (20 / 21 / 30) came from the embedded
                # team's firmware constants and are five orders of magnitude
                # below what this amplifier and cell should produce -- so they
                # were either derived against a very different setup, or never
                # finished.
                #
                # This case used to be silent, because the check above tests for
                # exactly 1.0 and nothing else. A number that is merely WRONG
                # looks calibrated, which is worse than one that admits it is a
                # placeholder: the weights come out self-consistent, repeatable
                # and off by a factor nobody is looking for.
                self.get_logger().warn(
                    f"cell '{name}' has scale {scale:g} counts/kg, which is far below "
                    f"the ~1e5 an HX711 at gain 128 should give. The weights will be "
                    f"repeatable but are probably not kilograms. Confirm it with one "
                    f"known mass: {self.ns}/{name}/raw, then set cell.{name}.scale.")
        ...
        # --- tare TOPICS, alongside the services above -------------------------
        #
        # Same operation, reachable without service plumbing:
        #
        #     ros2 topic pub --once /load_cells/sand_box/tare std_msgs/UInt8 "data: 1"
        #
        # The payload matches the convention the drill firmware uses for the
        # same job, so one number means one thing on both sides of the link:
        #
        #     1  tare EMPTY -- the cell as it stands right now is zero
        #     2  tare WITH LID -- what has been added since 1 is the lid
        #
        # WHY BOTH INTERFACES EXIST. A service answers, and that answer is the
        # whole point of the Trigger version: it reports the new offset, which
        # is the number to paste into load_cells.yaml. A topic cannot tell you
        # anything, so the offset is only logged. Use the service when you are
        # calibrating and the topic when you are operating.
        #
        # ANYTHING OTHER THAN 1 OR 2 IS IGNORED rather than guessed at. There is
        # no third sensible reading, and a mistyped value that silently re-zeroed
        # a cell mid-task would not show up until the numbers stopped adding up.
        for name in self.cell_names:
            self.create_subscription(
                UInt8, f"{self.ns}/{name}/tare",
                (lambda n: lambda msg: self._tare_topic_cb(n, msg))(name), 10)
    ...
    # -- tare --------------------------------------------------------------------
    def _tare_topic_cb(self, name, msg):
        """Topic-driven tare. See the note where these are created."""
        mode = int(msg.data)
        if mode not in (1, 2):
            return
        ...
        if mode == 1:
            offset = cell.tare()
            if offset is None:
                self.get_logger().warn(f"{name}: no counts to tare against")
                return
            cell.lid_offset = 0.0
            self.get_logger().info(
                f"Tared {name} empty: set cell.{name}.offset: {offset:.1f} in "
                f"load_cells.yaml to keep this across a restart")
            return

        # mode == 2: the lid. Measured as the difference from the empty zero,
        # so it is meaningless unless a mode-1 tare came first -- and it is
        # cleared by the next one, above, for exactly that reason.
        counts = cell.latest_counts()
        if counts is None:
            self.get_logger().warn(f"{name}: no counts to weigh the lid against")
            return
        cell.lid_offset = counts - cell.offset
        self.get_logger().info(
            f"Tared {name} with lid: lid is {cell.lid_offset:.1f} counts")
