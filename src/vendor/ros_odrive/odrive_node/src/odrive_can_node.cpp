...
ODriveCanNode::ODriveCanNode(const std::string& node_name) : rclcpp::Node(node_name) {
    ...
    //  - rclcpp pull request 713
    //  - rclcpp pull request 1969
...
void ODriveCanNode::recv_callback(const can_frame& frame) {
    ...
    // A zero-length frame for an ODrive GET command is a request, not a
    // response. The Aries status poller sends these periodically, and SocketCAN
    // loopback can deliver them to this node before the ODrive's 8-byte reply.
    if (
        frame.can_dlc == 0 &&
        (
            cmd_id == CmdId::kGetError ||
            cmd_id == CmdId::kGetEncoderEstimates ||
            cmd_id == CmdId::kGetIq ||
            cmd_id == CmdId::kGetTemp ||
            cmd_id == CmdId::kGetBusVoltageCurrent ||
            cmd_id == CmdId::kGetTorques
        )
    ) {
        return;
    }
