...
def resolve_imu_source(use_imu, imu_port, lidar_available):
    """Return ``(source, bno_available)`` for bno055, picoscan, or none."""
    ...
    if mode in FALSE_VALUES or mode in ("odom_only", "wheel_odom"):
        return "none", bno_available
    if mode in ("bno055", "serial"):
        return ("bno055" if bno_available else "none"), bno_available
    if mode in ("picoscan", "picoscan150", "sick", "lidar"):
        return ("picoscan" if lidar_available else "none"), bno_available

    if bno_available:
        return "bno055", bno_available
    if lidar_available:
        return "picoscan", bno_available
    return "none", bno_available
