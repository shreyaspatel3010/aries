...
def local_only_xml():
    ...
    No <Interfaces> pin, so Cyclone picks for itself, and multicast left on so
    ordinary single-machine discovery works. This is what a developer laptop
    running simulation needs -- there is no second machine to find, and pinning
    an interface it does not have is FATAL, not a warning: Cyclone refuses to
...
<!-- GENERATED by aries_common.comms on {socket.gethostname()}: this machine is
     NOT on the field link, so this is a local-only configuration. Domain and
     middleware still match the robot, so a simulation here and a rover on the
     antenna would still be on the same domain if they were ever bridged. -->
<CycloneDDS xmlns="https://cdds.io/config">
  <Domain id="any">
    <Discovery>
      <ParticipantIndex>auto</ParticipantIndex>
